...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:   decode-faster-ctc [options] <fst-in> <loglikes-rspecifier> <words-wspecifier> [<alignments-wspecifier>]\n";
    ...
    for (; !loglikes_reader.Done(); loglikes_reader.Next()) {
      ...
      DecodableMatrixScaledCtc decodable(loglikes, acoustic_scale);
      decoder.DecodeCtc(&decodable);
