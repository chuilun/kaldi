...
class GruStreams : public UpdatableComponent {
 ...
 public:
  ...
  void InitData(std::istream &is) {
    ...
    w_rz_r_.Resize(2*ncell_, ncell_, kUndefined);
    ...
    w_rz_r_corr_.Resize(2*ncell_, ncell_, kSetZero) ;
  ...
  void ResetGRUStreams(const std::vector<int32> &stream_reset_flag, int32 ntruncated_bptt_size) {
    ...
    if (nstream_ == 0) {
      ...
      prev_nnet_state_.Resize(nstream_, 4*ncell_ , kSetZero);
  ...
  void PropagateFnc(const CuMatrixBase<BaseFloat> &in, CuMatrixBase<BaseFloat> *out) {
    ...
    propagate_buf_.Resize((T+2)*S, 4 * ncell_ , kSetZero);
    propagate_buf_.RowRange(0*S,S).CopyFromMat(prev_nnet_state_);
    YRH_.Resize((T+2)*S, ncell_, kSetZero);
    DRH_.Resize((T+2)*S, ncell_, kSetZero);
    ...
    for (int t = 1; t <= T; t++) {
      ...
      CuSubMatrix<BaseFloat> y_rh(YRH_.RowRange(t*S,S));

      // h(t-1) -> r(t), z(t)
      y_rz.AddMatMat(1.0, YH.RowRange((t-1)*S,S), kNoTrans, w_rz_r_, kTrans,  1.0);

      // i, f sigmoid squashing
      y_rz.Sigmoid(y_rz);
      // r(t),h(t-1) -> rh(t)
      y_rh.AddMatMatElements(1.0, y_r, YH.RowRange((t-1)*S, S), 0.0);
      //rh -> c
      y_c.AddMatMat(1.0, y_rh, kNoTrans, w_c_r_, kTrans, 1.0);
  ...
  void BackpropagateFnc(const CuMatrixBase<BaseFloat> &in, const CuMatrixBase<BaseFloat> &out,
    ...
    backpropagate_buf_.Resize((T+2)*S, 4 * ncell_ , kSetZero);
    ...
    for (int t = T; t >= 1; t--) {
      ...
      d_h.AddMatMat(1.0, DRZ.RowRange((t+1)*S,S), kNoTrans, w_rz_r_, kNoTrans, 1.0);

      DRH_.RowRange((t+1)*S,S).AddMatMat(1.0, DC.RowRange((t+1)*S,S), kNoTrans, w_c_r_, kNoTrans, 0.0);
      DRH_.RowRange((t+1)*S,S).MulElements(YR.RowRange((t+1)*S,S)) ;

      d_h.AddMat(1.0, DRH_.RowRange((t+1)*S,S)) ;
      ...
      //c -> r
      d_rh.AddMatMat(1.0, d_c, kNoTrans, w_c_r_, kNoTrans, 0.0);
      d_r.AddMatMatElements(1.0, YH.RowRange((t-1)*S,S) , d_rh, 0.0);
 ...
 private:
  ...
  CuMatrix<BaseFloat> YRH_;

  CuMatrix<BaseFloat> DRH_ ;
