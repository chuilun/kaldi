...
#ifndef KALDI_NNET_NNET_GRU_PROJECTED_STREAMS_FAST_H_
#define KALDI_NNET_NNET_GRU_PROJECTED_STREAMS_FAST_H_
...
class GruProjectedStreamsFast : public UpdatableComponent {
	friend class NnetModelSync;
 public:
  GruProjectedStreamsFast(int32 input_dim, int32 output_dim) :
  ...
  ~GruProjectedStreamsFast()
  { }

  Component* Copy() const { return new GruProjectedStreamsFast(*this); }
  ComponentType GetType() const { return kGruProjectedStreamsFast; }
  ...
  void InitData(std::istream &is) {
    ...
    w_r_x_.Resize(nrecur_, input_dim_, kUndefined);
    w_zc_x_.Resize(2*ncell_, input_dim_, kUndefined);
    w_r_r_.Resize(nrecur_, nrecur_, kUndefined);
    w_z_r_.Resize(ncell_, nrecur_, kUndefined);
    w_c_r_.Resize(ncell_, nrecur_, kUndefined);
    w_p_m_.Resize(nrecur_, ncell_, kUndefined);
    InitMatParam(w_r_x_, param_scale);
    InitMatParam(w_zc_x_, param_scale);
    InitMatParam(w_r_r_, param_scale);
    InitMatParam(w_z_r_, param_scale);
    InitMatParam(w_c_r_, param_scale);
    InitMatParam(w_p_m_, param_scale);

    bias_r_.Resize(nrecur_, kUndefined);
    bias_zc_.Resize(2*ncell_, kUndefined);
    InitVecParam(bias_r_, param_scale);
    InitVecParam(bias_zc_, param_scale);

    // init delta buffers
    w_r_x_corr_.Resize(nrecur_, input_dim_, kSetZero);
    w_zc_x_corr_.Resize(2*ncell_, input_dim_, kSetZero);
    w_r_r_corr_.Resize(nrecur_, nrecur_, kSetZero) ;
    w_z_r_corr_.Resize(ncell_, nrecur_, kSetZero) ;
    w_c_r_corr_.Resize(ncell_, nrecur_, kSetZero) ;
    w_p_m_corr_.Resize(nrecur_, ncell_, kSetZero) ;
    bias_r_corr_.Resize(nrecur_, kSetZero);
    bias_zc_corr_.Resize(2*ncell_, kSetZero);
  ...
  void ReadData(std::istream &is, bool binary) {
    ...
    w_zc_x_.Read(is, binary);
    w_r_r_.Read(is, binary);
    w_z_r_.Read(is, binary);
    w_c_r_.Read(is, binary);
    w_p_m_.Read(is, binary);
    bias_r_.Read(is, binary);
    bias_zc_.Read(is, binary);
  ...
  void WriteData(std::ostream &os, bool binary) const {
    ...
    w_zc_x_.Write(os, binary);
    w_r_r_.Write(os, binary);
    w_z_r_.Write(os, binary) ;
    w_c_r_.Write(os, binary);
    w_p_m_.Write(os, binary);
    bias_r_.Write(os, binary);
    bias_zc_.Write(os, binary);

  }

  int32 NumParams() const {
    return ( w_r_x_.NumRows() * w_r_x_.NumCols() +
         w_zc_x_.NumRows() * w_zc_x_.NumCols() +
         w_r_r_.NumRows() * w_r_r_.NumCols() +
         w_z_r_.NumRows() * w_z_r_.NumCols() +
        w_c_r_.NumRows() * w_c_r_.NumCols() +
        w_p_m_.NumRows() * w_p_m_.NumCols() +
         bias_r_.Dim() + bias_zc_.Dim()) ;
  }
  ...
  void GetParams(Vector<BaseFloat>* wei_copy) const {
    ...
    offset = 0;len = w_r_x_.NumRows() * w_r_x_.NumCols();
    wei_copy->Range(offset, len).CopyRowsFromMat(w_r_x_);
    offset += len; len = w_zc_x_.NumRows() * w_zc_x_.NumCols();
    wei_copy->Range(offset, len).CopyRowsFromMat(w_zc_x_);

    offset += len; len = w_r_r_.NumRows() * w_r_r_.NumCols();
    wei_copy->Range(offset, len).CopyRowsFromMat(w_r_r_);

    offset += len ; len = w_z_r_.NumRows() * w_z_r_.NumCols() ;
    wei_copy->Range(offset, len).CopyRowsFromMat(w_z_r_);
    ...
    wei_copy->Range(offset, len).CopyFromVec(bias_r_);
    offset += len; len = bias_zc_.Dim();
    wei_copy->Range(offset, len).CopyFromVec(bias_zc_);
  ...
  std::string Info() const {
    return std::string("  ") +
      ...
      "\n  w_zc_x_  "   + MomentStatistics(w_zc_x_) +
      "\n  w_r_r_  "   + MomentStatistics(w_r_r_) +
      "\n  w_z_r_ " + MomentStatistics(w_z_r_) +
      "\n  w_c_r_ " + MomentStatistics(w_c_r_) +
      "\n  w_p_m_" + MomentStatistics(w_p_m_)+
      "\n  bias_r_  " + MomentStatistics(bias_r_) +
      "\n  bias_zc_ " + MomentStatistics(bias_zc_);
  }

  std::string InfoGradient() const {
    // disassemble forward-propagation buffer into different neurons,
    const CuSubMatrix<BaseFloat> YR(propagate_buf_.ColRange(0*ncell_, nrecur_));
    ...
    const CuSubMatrix<BaseFloat> DR(backpropagate_buf_.ColRange(0*ncell_, nrecur_));
    ...
    return std::string("  ") +
      ...
      "\n  w_zc_x_corr_  "   + MomentStatistics(w_zc_x_corr_) +
      "\n  w_r_r_corr_  "   + MomentStatistics(w_r_r_corr_) +
      "\n  w_z_r_corr_"  + MomentStatistics(w_z_r_corr_) +
      ...
      "\n  bias_zc_corr_ " +  MomentStatistics(bias_zc_corr_) +
      "\n  Forward-pass:" +
  ...
  void ResetGRUProjectedStreamsFast(const std::vector<int32> &stream_reset_flag, int32 ntruncated_bptt_size) {
  ...
  void PropagateFnc(const CuMatrixBase<BaseFloat> &in, CuMatrixBase<BaseFloat> *out) {
    ...
    YRP_.Resize((T+2)*S, nrecur_, kSetZero);
    DRP_.Resize((T+2)*S, nrecur_, kSetZero);
    // disassemble entire neuron activation buffer into different neurons
    CuSubMatrix<BaseFloat> YR(propagate_buf_.ColRange(0*ncell_, nrecur_));
    ...
    CuSubMatrix<BaseFloat> YZC(propagate_buf_.ColRange(1*ncell_, 2*ncell_));

    // x -> r, z, c not recurrent, 
    YR.RowRange(1*S,T*S).AddMatMat(1.0, in, kNoTrans, w_r_x_, kTrans, 0.0);
    YZC.RowRange(1*S,T*S).AddMatMat(1.0, in, kNoTrans, w_zc_x_, kTrans, 0.0);
    // bias -> r, z, c
    YR.RowRange(1*S,T*S).AddVecToRows(1.0, bias_r_);
    YZC.RowRange(1*S,T*S).AddVecToRows(1.0, bias_zc_);
    ...
    for (int t = 1; t <= T; t++) {
      ...
      // p(t-1) -> r(t)
      y_r.AddMatMat(1.0, YP.RowRange((t-1)*S,S), kNoTrans, w_r_r_, kTrans,  1.0);
      y_r.Sigmoid(y_r);
      // p(t-1) -> z(t)
      y_z.AddMatMat(1.0, YP.RowRange((t-1)*S,S), kNoTrans, w_z_r_, kTrans,  1.0);
      y_z.Sigmoid(y_z);
      // r(t),p(t-1) -> rp(t)
      y_rp.AddMatMatElements(1.0, y_r, YP.RowRange((t-1)*S, S), 0.0);
      //rp -> c
  ...
  void BackpropagateFnc(const CuMatrixBase<BaseFloat> &in, const CuMatrixBase<BaseFloat> &out,
    ...
    CuSubMatrix<BaseFloat> DR(backpropagate_buf_.ColRange(0*ncell_, nrecur_));
    ...
    CuSubMatrix<BaseFloat> DZC(backpropagate_buf_.ColRange(1*ncell_, 2*ncell_));
    ...
    for (int t = T; t >= 1; t--) {
      ...
      //   backprop error from r(t+1), z(t+1)  c(t+1) to p(t)
      d_p.AddMatMat(1.0, DR.RowRange((t+1)*S,S), kNoTrans, w_r_r_, kNoTrans, 1.0);
      d_p.AddMatMat(1.0, DZ.RowRange((t+1)*S,S), kNoTrans, w_z_r_,  kNoTrans, 1.0);
      ...
      d_p.AddMat(1.0, DRP_.RowRange((t+1)*S,S));
      // h(t+1), p(t)->h(t)
      ...
      d_r.AddMatMatElements(1.0, YP.RowRange((t-1)*S,S) , d_rp, 0.0);
    ...
    in_diff->AddMatMat(1.0, DZC.RowRange(1*S,T*S), kNoTrans, w_zc_x_, kNoTrans, 0.0);
  ...
  void Gradient(const CuMatrixBase<BaseFloat> &input, const CuMatrixBase<BaseFloat> &diff)
  {
	    ...
	    // weight x -> r , z , c
	    w_r_x_corr_.AddMatMat(1.0, DR.RowRange(1*S,T*S), kTrans, input, kNoTrans, mmt);
	    w_zc_x_corr_.AddMatMat(1.0, DZC.RowRange(1*S,T*S), kTrans,
	                                  input                     , kNoTrans, mmt);
	    // recurrent weight h -> r , z
	    w_r_r_corr_.AddMatMat(1.0, DR.RowRange(1*S,T*S), kTrans,
	                                  YP.RowRange(0*S,T*S)   , kNoTrans, mmt );
        w_z_r_corr_.AddMatMat(1.0, DZ.RowRange(1*S,T*S), kTrans,
                                      YP.RowRange(0*S,T*S)   , kNoTrans, mmt );
	    ...
	    bias_zc_corr_.AddRowSumMat(1.0, DZC.RowRange(1*S,T*S), mmt);

	    if (clip_gradient_ > 0.0) {

	      w_r_x_corr_.ApplyFloor(-clip_gradient_);
	      w_r_x_corr_.ApplyCeiling(clip_gradient_);
          w_zc_x_corr_.ApplyFloor(-clip_gradient_);
          w_zc_x_corr_.ApplyCeiling(clip_gradient_);
	      w_r_r_corr_.ApplyFloor(-clip_gradient_);
	      w_r_r_corr_.ApplyCeiling(clip_gradient_);
	      w_z_r_corr_.ApplyFloor(-clip_gradient_);
          w_z_r_corr_.ApplyCeiling(clip_gradient_);
	      ...
	      bias_r_corr_.ApplyFloor(-clip_gradient_);
	      bias_r_corr_.ApplyCeiling(clip_gradient_);
          bias_zc_corr_.ApplyFloor(-clip_gradient_);
          bias_zc_corr_.ApplyCeiling(clip_gradient_);
	    }

	    if (DEBUG) {
	      std::cerr << "gradients(with optional momentum): \n";
	      std::cerr << "w_r_x_corr_ " << w_r_x_corr_;
          std::cerr << "w_zc_x_corr_ " << w_zc_x_corr_;
	      std::cerr << "w_r_r_corr_ " << w_r_r_corr_;
          std::cerr << "w_z_r_corr_" << w_z_r_corr_;
          std::cerr << "w_c_r_corr_" << w_c_r_corr_;
	      std::cerr << "bias_r_corr_ " << bias_r_corr_;
          std::cerr << "bias_zc_corr_" << bias_zc_corr_;

	    }

		 // l2 regularization
		 if (l2 != 0.0) {
			 w_r_x_.AddMat(-lr*l2*num_frames, w_r_x_);
             w_zc_x_.AddMat(-lr*l2*num_frames, w_zc_x_);
			 w_r_r_.AddMat(-lr*l2*num_frames, w_r_r_);
             w_z_r_.AddMat(-lr*l2*num_frames, w_z_r_);
  ...
  void UpdateGradient()
  {
        ...
        w_zc_x_.AddMat(-lr, w_zc_x_corr_);
        w_r_r_.AddMat(-lr, w_r_r_corr_);
        w_z_r_.AddMat(-lr, w_z_r_corr_);
        w_c_r_.AddMat(-lr, w_c_r_corr_);
        w_p_m_.AddMat(-lr, w_p_m_corr_);
        bias_r_.AddVec(-lr_bias, bias_r_corr_, 1.0);
        bias_zc_.AddVec(-lr_bias, bias_zc_corr_, 1.0);
  }
  ...
  void Update(const CuMatrixBase<BaseFloat> &input, const CuMatrixBase<BaseFloat> &diff) {
    ...
    w_c_r_.AddMat(-lr, w_c_r_corr_);
    w_p_m_.AddMat(-lr, w_p_m_corr_);
    bias_r_.AddVec(-lr, bias_r_corr_, 1.0);
    bias_zc_.AddVec(-lr, bias_zc_corr_, 1.0);
 ...
 private:
  ...
  // feed-forward connections: from x to 
  CuMatrix<BaseFloat> w_r_x_ ;
  CuMatrix<BaseFloat> w_r_x_corr_;
  CuMatrix<BaseFloat> w_zc_x_;
  CuMatrix<BaseFloat> w_zc_x_corr_;

  // recurrent projection connections: from r to [g, i, f, o]
  CuMatrix<BaseFloat> w_r_r_;
  CuMatrix<BaseFloat> w_r_r_corr_;
  
  CuMatrix<BaseFloat> w_z_r_;
  CuMatrix<BaseFloat> w_z_r_corr_; 
  ...
  CuVector<BaseFloat> bias_zc_;
  CuVector<BaseFloat> bias_zc_corr_;
