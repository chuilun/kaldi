...
bool LatticeBoostCTC(const std::vector<int32> &alignment,
  ...
  for (int32 state = 0; state < num_states; state++) {
    ...
    for (fst::MutableArcIterator<Lattice> aiter(lat, state); !aiter.Done();
      ...
      if (arc.ilabel != 0) {  // Non-epsilon arc
        if (arc.ilabel < 0) {
          KALDI_WARN << "Lattice has out-of-range pdf-ids: "
                     << "lattice/model mismatch?";
          return false;
        }
        int32 phone = arc.ilabel, //trans.TransitionIdToPhone(arc.ilabel),
            ref_phone = alignment[cur_time]; // trans.TransitionIdToPhone(alignment[cur_time]);
        BaseFloat frame_error;
        if (phone == ref_phone) {
          frame_error = 0.0;
        } else { // an error...
...
BaseFloat LatticeForwardBackwardMpeVariantsCTC(
    const std::vector<int32> &silence_phones,
    const Lattice &lat,
    const std::vector<int32> &num_ali,
    std::string criterion,
  ...
  for (StateId s = 0; s < num_states; s++) {
    ...
    for (ArcIterator<Lattice> aiter(lat, s); !aiter.Done(); aiter.Next()) {
      ...
      if (arc.ilabel != 0) {
        int32 cur_time = state_times[s];
        int32 phone = arc.ilabel, //trans.TransitionIdToPhone(arc.ilabel)
            ref_phone = num_ali[cur_time]; //trans.TransitionIdToPhone(num_ali[cur_time]);
        bool phone_is_sil = std::binary_search(silence_phones.begin(),
                                               silence_phones.end(),
                                               phone),
            ref_phone_is_sil = std::binary_search(silence_phones.begin(),
            ...
            both_sil = phone_is_sil && ref_phone_is_sil;
        if (!is_mpfe) { // smbr.
          int32 pdf = arc.ilabel, //trans.TransitionIdToPdf(arc.ilabel),
              ref_pdf = num_ali[cur_time]; //trans.TransitionIdToPdf(num_ali[cur_time]);
          if (!one_silence_class)  // old behavior
            frame_acc = (pdf == ref_pdf && !phone_is_sil) ? 1.0 : 0.0;
          else
            frame_acc = (pdf == ref_pdf || both_sil) ? 1.0 : 0.0;
