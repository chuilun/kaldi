...
#   pragma unroll
  ...
  Real sum = smem[0];
  if (threadIdx.x == 0 && logsum != NULL) logsum[i] = log(sum)+max; // denominator log sum
...
void cudaF_apply_exp(dim3 Gr, dim3 Bl, float* mat, MatrixDim d, cudaStream_t s) {
...
void cudaF_add_mat(dim3 Gr, dim3 Bl, float alpha, const float* src, float* dst,
                   MatrixDim d, int src_stride, int A_trans, cudaStream_t s) {
  if (A_trans) {
...
void cudaF_softmax_reduce(size_t Gr, size_t Bl, float* y, const float* x,
                          MatrixDim d, int src_stride, float *logsum, cudaStream_t s) {
...
void cudaD_apply_exp(dim3 Gr, dim3 Bl, double* mat, MatrixDim d, cudaStream_t s) {
  _apply_exp<<<Gr,Bl,0,s>>>(mat,d);
}
...
void cudaD_add_mat(dim3 Gr, dim3 Bl, double alpha, const double* src,
                   double* dst, MatrixDim d, int src_stride, int A_trans, cudaStream_t s) {
  if (A_trans) {
    _add_mat_trans<<<Gr,Bl,0,s>>>(alpha,src,dst,d,src_stride);
  } else {
    _add_mat<<<Gr,Bl,0,s>>>(alpha,src,dst,d,src_stride);
...
void cudaD_softmax_reduce(size_t Gr, size_t Bl, double* y, const double* x,
                          MatrixDim d, int src_stride, double *logsum, cudaStream_t s) {
  _softmax_reduce<<<Gr,Bl,0,s>>>(y, x, d, src_stride, logsum);
}
...
/*
 * All the following kernels are written by Wei Deng for LSTM LM and convolution neural network training
 */
...
template<typename Real>
__global__
static void _cross_entropy(Real *xentropy, const Real *nnetout, 
							const Real *target, MatrixDim dst_d, int out_stride, int tgt_stride) {
  int32_cuda i = blockIdx.x * blockDim.x + threadIdx.x;
  int32_cuda j = blockIdx.y * blockDim.y + threadIdx.y;
